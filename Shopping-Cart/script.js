...
function addToCart(productId) {
    ...
    if (product) {
        cart.push({
            ...
            name: product.name,
            price: product.price,
            location: product.location // Include location data
        });
        updateCartDisplay();
    }
}
...
function proceedToFloorMap() {
    // Format cart data for the floor map
    const mapData = cart.map(item => ({
        name: item.name,
        location: item.location
    }));
    
    // Save to localStorage
    localStorage.setItem('selectedItems', JSON.stringify(mapData));
